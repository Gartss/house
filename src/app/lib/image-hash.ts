...
export function sha256HexPortable(input: Uint8Array) {
...
export async function sha256Hex(buffer: ArrayBuffer) {
  if (globalThis.crypto?.subtle) {
    try {
      ...
      // Some HTTP mobile webviews expose crypto but reject subtle operations.
    }
  }
  return sha256HexPortable(new Uint8Array(buffer));
}
